...
contract VotingSystem{
    ...
    event signUpVoter(address indexed voterAddress, string indexed cnp);
    ...
    function SignUpVoter(string memory cnp) external{
        ...
        emit signUpVoter(msg.sender, cnp);
    }
    ...
    function GetAllProjects() view external returns(ProjectDTO[] memory){
        uint256 numberProjects = projects.length;
        ProjectDTO[] memory projectsDTO = new ProjectDTO[](numberProjects);
        for(uint256 i=0; i<numberProjects; ++i){
            Project storage project = projects[i];
            ProjectDTO memory projectDTO = ProjectDTO(
                project.name,
                project.optionSize,
                project.availableFunds,
                project.choiceDeadline
            );
            projectsDTO[i] = projectDTO;
        }
        return projectsDTO;
    }

    function GetAllOptionsForProject(uint256 projectId) view external returns(OptionDto[] memory){
        uint optionNumber = projects[projectId].optionSize;
        OptionDto[] memory optionsDTO = new OptionDto[](optionNumber);
        for(uint256 i=0; i<optionNumber; ++i){
            Option storage option = projects[projectId].options[i];
            OptionDto memory optionDTO = OptionDto(
                option.name,
                option.description,
                option.cost,
                option.voteCount,
